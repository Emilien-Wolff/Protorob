...
#define NUM_LEDS_TEMP   9    // affichage température
#define LED_TYPE       WS2812B
#define COLOR_ORDER    GRB
#define LED_OFFSET     3     // décalage horaire

CRGB leds[NUM_LEDS_TOTAL];

// ================== SYSTEME MECANIQUE ==================
#define ENCODER_PPR     12    // nbr de fentes dans l'encodeur par tour
#define QUAD_FACTOR     4     // fronts montants et descendants
#define GEAR_RATIO      19.0
#define NEEDLE_DEG      215.0 // amplitude en ° entre la position max et min
...
ISR(TIMER1_COMPA_vect) {
  ...
  integral += error;
  integral = constrain(integral, -INTEGRAL_LIMIT, INTEGRAL_LIMIT);  // partie intégrale bornée

  float derivative = error - prevError;                     // calcul de variation de l'erreur
  prevError = error;

  output = Kp * error + Ki * integral + Kd * derivative;    // calcul du PID
  output = constrain(output, -PWM_MAX, PWM_MAX);            // sortie bornée
  ...
  if (abs(error) < 2) {
    ...
    digitalWrite(PIN_DIR, (error > 0) ? LOW : HIGH);  // si l'erreur est position, on tourne dans un sens
...
void calibrateNeedle() {
  ...
  digitalWrite(PIN_DIR, HIGH);      // on fait tourner l'aiguille vers la gauche pour aller sur la butée

  analogWrite(PIN_PWM, 60);             // on lance le moteur à la vitesse min normale
  delay(100);
  analogWrite(PIN_PWM, 35);             // on réduit pour se mettre au plus lent et détecter rapidement la butée

  while (stableCount < 3) {

    if (millis() - lastBlink > 250) {   // on change l'état des leds toutes les 250ms
      lastBlink = millis();
      ledState = !ledState;
    ...
    if (delta <= 1) stableCount++;      // l'aiguille est bloquée 3 fois de suite pour sortir de la boucle while
    else stableCount = 0;
  }

  analogWrite(PIN_PWM, 0);              // arrêt du moteur
  encoder.write(0);                     // initialisation du codeur

  FastLED.clear();                      // on éteint les leds
...
void updateTemperatureRing(float temperature) {
  ...
  for (int i = 0; i < NUM_LEDS_TEMP; i++) {
    ...
    int ledIndex = (i + LED_OFFSET) % NUM_LEDS_TEMP;

    if (i < activeLeds) {
      uint8_t hue = map(i, 0, NUM_LEDS_TEMP - 1, 160, 0);
...
void setup() {
  ...
  calibrateNeedle();                  // recherche de la butée
...
void loop() {
  ...
  float temp = tempSensor.getTemperature(); // mesure de la température
  samples.add(temp);                        // stockage dans la variable du PID

  float temperature = samples.getMedian();  // lissage pour éviter les glitchs
  temperature = constrain(temperature, TEMP_LIMIT_MIN, TEMP_MAX);

  targetPosition = map(       // définition des correspondances entre deux intervalles
    (int)temperature,
    (int)TEMP_LIMIT_MIN,      // intervalle de températures
    (int)TEMP_MAX,
    0,                        // intervalle des angles
    TICKS_MAX
  );
  targetPosition = constrain(targetPosition, 0, TICKS_MAX); // limitation dans un intervalle réaliste
                                                            // par sécurité matérielle
